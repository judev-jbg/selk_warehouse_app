...
import 'load_order_detail_page.dart';
...
class PickingPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create:
          (context) => PickingBloc(
            getLoadOrders: context.read(),
            getLoadOrderDetail: context.read(),
            startPickingProcess: context.read(),
          )..add(const GetLoadOrdersEvent()),
      child: _PickingPageContent(),
    );
  }
}

class _PickingPageContent extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
  ...
  Widget _buildStatusFilter(BuildContext context, PickingState state) {
    return Container(
  ...
  Widget _buildContent(BuildContext context, PickingState state) {
    if (state is LoadOrdersLoaded) {
      return ListView.builder(
        padding: EdgeInsets.all(16),
        itemCount: state.loadOrders.length,
        itemBuilder: (context, index) {
          final loadOrder = state.loadOrders[index];
          return _buildLoadOrderCard(context, loadOrder);
        },
      );
  ...
  Widget _buildLoadOrderCard(BuildContext context, LoadOrder loadOrder) {
    return Card(
      ...
      child: InkWell(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => LoadOrderDetailPage(loadOrderId: loadOrder.id),
            ),
          );
        },
